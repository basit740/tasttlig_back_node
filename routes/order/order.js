...
router.get("/orders/user/:userId", token_service.authenticateToken,  async (req, res) => {
  try {
...
router.get("/orders/current/user/:userId", token_service.authenticateToken,  async (req, res) => {
  try {
    const userOrders = await order_service.getAllCurrentOrders(
      req.user.id
    );
